function cardGame(input) {
    let symbolToPints = { 'J': 11, 'Q': 12, 'K': 13, 'A': 14, 'S': 4, 'H': 3, 'D': 2, 'C': 1 }
    let players = {};
    let results = {};
    ...
    for (let [playerName, playerCards] of Object.entries(players)) {
        ...
        for (let i = 0; i < playerCards.length; i++) {
            ...
            if (playerCards.indexOf(card) === i) {
                ...
                if (Object.keys(symbolToPints).includes(power)) {
                    cardPoints = symbolToPints[power] * symbolToPints[type]
                } else {
                    cardPoints = Number(power) * symbolToPints[type]
                }
